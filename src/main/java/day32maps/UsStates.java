...
public enum UsStates {
    ...
    public static String getStateNameFromAbbreviation(String abbreviation){
        return Arrays.stream(values()).filter(UsStates -> UsStates.getAbbreviation().equals(abbreviation)).findFirst().get().getStateName(); // get() ile findFirst() methodunun verdigi Optional i String e ceviriyoruz.
        // Arrays.stream(values()) method u enum daki butun data yi stream e cevirir.
    }
    ...
    // Example 1: Baskenti verilen state in kısaltmasını ekrana yazdıran method u olusturunuz.
    ...
    public static String getCapitalFromAbbreviation(String abbreviation){
        return Arrays.stream(values()).filter(t -> t.getAbbreviation().equals(abbreviation)).findFirst().get().getCapital();
    }
